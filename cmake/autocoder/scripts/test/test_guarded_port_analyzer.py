...
from guarded_port_analyzer import (
    FindingKind,
    GuardedPortAnalyzer,
    Severity,
    SyncKind,
...
def analyze(model_dir, **kwargs):
    """Run the analyzer over a built model directory"""
    analyzer = GuardedPortAnalyzer(topology_path=model_dir, **kwargs)
    analyzer.run()
    return analyzer
...
def test_abba_between_two_guarded_components(model_builder):
    """Two all-guarded components reaching into each other deadlock"""
    analyzer = analyze(model_builder("synthetic_abba"))
...
def test_async_hop_breaks_the_lock_chain(model_builder):
...
def test_sync_passthrough_preserves_the_lock_chain(model_builder):
    """A sync port takes no mutex but keeps the chain on the caller's thread"""
    analyzer = analyze(model_builder("synthetic_sync_passthrough"))
...
def test_one_way_guarded_chain_is_clean(model_builder):
    """Locks taken in a single consistent order are not a hazard"""
    analyzer = analyze(model_builder("synthetic_clean"))
...
def test_suppression_removes_a_lock_edge(model_builder):
    """A recorded lock ordering can be suppressed once it is enforced"""
    analyzer = analyze(
        model_builder("synthetic_abba"), suppressions={("T.a", "T.b")}
    )
...
def test_real_tlmchan_guarded_ports_are_recognized(model_builder):
    """Svc.TlmChan's shipped port kinds are read correctly from the model"""
    analyzer = analyze(model_builder("real_tlmchan_cycle"))
...
def test_real_tlmchan_cycle_is_detected(model_builder):
...
def test_real_buffermanager_topology_is_clean(model_builder):
    """A correctly wired real BufferManager produces no findings"""
    analyzer = analyze(model_builder("real_buffermanager_clean"))
...
def test_real_event_manager_log_recv_is_sync(model_builder):
